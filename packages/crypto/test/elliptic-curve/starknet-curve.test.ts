...
describe("Starknet Elliptic Curve Primitives", () => {
  ...
  describe("getBasePointStarknet", () => {
    it("should return the uncompressed base point G by default", () => {
      ...
      const G_point = STARKNET_CURVE.ProjectivePoint.fromHex(encode.removeHexPrefix(G_hex));
    ...
    it("should return the compressed base point G when specified", () => {
      ...
      const G_point_from_compressed = STARKNET_CURVE.ProjectivePoint.fromHex(encode.removeHexPrefix(G_hex_compressed));
      expect(G_point_from_compressed.equals(STARKNET_CURVE.ProjectivePoint.BASE)).toBe(true);
    });
  });
  ...
  describe("getPublicKeyStarknet", () => {
    ...
    it("should derive a known public key from a known private key", () => {
      const knownPrivHex = "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abc";
      const expectedKnownPubHexUncompressed = STARKNET_CURVE.getPublicKey(knownPrivHex, false);
      const derivedPubHexUncompressed = getPublicKeyStarknet(knownPrivHex, false);
      expect(derivedPubHexUncompressed.toLowerCase()).toEqual(encode.addHexPrefix(encode.buf2hex(expectedKnownPubHexUncompressed)).toLowerCase());

      const expectedKnownPubHexCompressed = STARKNET_CURVE.getPublicKey(knownPrivHex, true);
      const derivedPubHexCompressed = getPublicKeyStarknet(knownPrivHex, true);
      expect(derivedPubHexCompressed.toLowerCase()).toEqual(encode.addHexPrefix(encode.buf2hex(expectedKnownPubHexCompressed)).toLowerCase());
    });
  });
  ...
  describe("scalarMultiplyStarknet", () => {
    ...
    it("should handle multiplication by curve order (n * P = O, point at infinity)", () => {
      ...
      expect(resultPoint.toLowerCase()).toEqual(POINT_AT_INFINITY_HEX_UNCOMPRESSED.toLowerCase());
    });
