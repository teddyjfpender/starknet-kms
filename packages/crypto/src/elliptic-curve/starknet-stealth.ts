 ...
 */
export function createStealthAddressStarknet(recipientPubSpendKeyHex: string, recipientPubViewKeyHex: string) {
  ...
  const S_sharedSecretPointHex = scalarMultiplyStarknet(r_scalarHex, recipientPubViewKeyHex);

  // 4. Compute k = Hash(S_compressed)
  //    S_compressed is the compressed form of the shared secret point S
  const S_point = STARKNET_CURVE.ProjectivePoint.fromHex(encode.removeHexPrefix(S_sharedSecretPointHex));
  ...
  const P_stealthAddressHex = addPointsStarknet(recipientPubSpendKeyHex, P_stealth_intermediateHex);
...
export function checkStealthAddressOwnershipStarknet(
  ...
  const S_prime_sharedSecretPointHex = scalarMultiplyStarknet(recipientPrivateViewKeyHex, ephemeralPublicKeyHex);

  // 2. Compute k' = Hash(S'_compressed)
  const S_prime_point = STARKNET_CURVE.ProjectivePoint.fromHex(encode.removeHexPrefix(S_prime_sharedSecretPointHex));
  ...
  const k_prime_hashedBigInt = num.toBigInt(hash.starknetKeccak(k_prime_hashInputHex));
  ...
  //    (X is recipientPubSpendKeyHex)
  const P_prime_candidateHex = addPointsStarknet(recipientPubSpendKeyHex, P_prime_candidate_intermediateHex);
...
export function deriveStealthPrivateKeyStarknet(
  ...
  const x_recipientPrivateSpendScalar = num.toBigInt(recipientPrivateSpendKeyHex);

  // 4. Compute stealth private key p_stealth = (x + k') mod n
  const p_stealth_scalar = (x_recipientPrivateSpendScalar + k_prime_scalar) % CURVE_ORDER;
