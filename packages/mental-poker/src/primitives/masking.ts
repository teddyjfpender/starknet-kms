...
export function mask(
  ...
  const { ciphertext, proof: elgamalProof } = ElGamal.encrypt(elgamalParams, publicKey, plaintext, r)
...
export function verifyMask(
  ...
  return ElGamal.verifyEncryption(elgamalParams, publicKey, plaintext, ciphertext, elgamalProof)
}
...
export function remask(
  ...
  const remaskedCiphertext = ElGamal.addCiphertexts(originalCiphertext, zeroEncryption)
 ...
 */
export function maskedCardsEqual(masked1: MaskedCard, masked2: MaskedCard): boolean {
  return arePointsEqual(masked1.c1, masked2.c1) && arePointsEqual(masked1.c2, masked2.c2)
}
