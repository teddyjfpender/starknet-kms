...
import { randScalar, scalarMultiply, addPoints, type Point, CURVE_ORDER, moduloOrder, PRIME, generateChallenge } from '@starkms/crypto'
...
function generateRevealTestVector(seed: number[], testIndex: number): RevealTestVector {
...
function generateRevealTestVectors(count: number = 3): void {
  // Use the same seeds as the Rust reference for consistency
  const seeds = [
    Array(32).fill(0).map((_, i) => 42 + i), // [42, 43, 44, ...]
    Array(32).fill(0).map((_, i) => i), // [0, 1, 2, ...]
    Array(32).fill(0).map((_, i) => 100 + i * 2), // [100, 102, 104, ...]
  ]
  ...
  const output: RevealTestVectors = {
    protocol_info: {
      ...
      version: "1.0.0",
      description: "Test vectors for reveal operations in the Barnett-Smart protocol"
    },
    curve_info: {
      name: "Starknet Curve",
  ...
  console.log(`Generated ${testVectors.length} reveal test vectors in ${outputPath}`)
}

// Main execution
const args = process.argv.slice(2)
const count = args.length > 0 ? parseInt(args[0]!) : 3

if (isNaN(count) || count <= 0) {
  console.error('Please provide a valid positive number of test vectors to generate')
  process.exit(1)
}
