...
import {
  ...
  hexToScalar,
  hexToPoint,
  vectorPermutationToPermutation,
} from './vector-helpers';
...
describe('Mental Poker Vector Reproduction Tests', () => {
  ...
  beforeAll(async () => {
    ...
    for (const playerName of playerNames) {
      const [pk, sk] = vectorPlayerToKeys((testVector.players as any)[playerName]);
    ...
    for (const [playerName, [pk, sk]] of playerKeys) {
      ...
      const proof = await dlCards.proveKeyOwnership(parameters, pk, sk, playerInfo);
      playerKeysProofInfo.push([pk, proof, playerInfo] as const);
    }
    
    aggregateKey = await dlCards.computeAggregateKey(parameters, playerKeysProofInfo);
  ...
  describe('Structural Validation', () => {
    ...
    it('should have final results for all players', () => {
      ...
      expect(Object.keys(finalResults)).toEqual(['andrija', 'kobi', 'nico', 'tom']);
  ...
  describe('Key Generation Reproduction', () => {
    it('should use exact vector secret keys', () => {
      const playerNames = getPlayerNames(testVector);
         ...
         const expectedSk = hexToScalar((testVector.players as any)[playerName].secret_key_hex);
    ...
    it('should verify key ownership with vector keys', async () => {
      ...
      for (const playerName of playerNames) {
        ...
        const proof = await dlCards.proveKeyOwnership(parameters, pk, sk, playerInfo);
        const isValid = await dlCards.verifyKeyOwnership(parameters, pk, playerInfo, proof);
  ...
  describe('Card Masking Reproduction', () => {
    it('should mask cards using vector data', async () => {
      ...
      for (const { card, playingCard } of testCards) {
        // Use a deterministic masking factor
        const alpha = rng.nextScalar();
        
        const [maskedCard, maskProof] = await dlCards.mask(parameters, aggregateKey, card, alpha);
        
        // Verify the masking proof
        const isValid = await dlCards.verifyMask(parameters, aggregateKey, card, maskedCard, maskProof);
        expect(isValid).toBe(true);
    ...
    it('should remask cards correctly', async () => {
      ...
      // Initial masking
      const [maskedCard, maskProof] = await dlCards.mask(parameters, aggregateKey, testCard, alpha1);
      
      // Remasking
      const [remaskedCard, remaskProof] = await dlCards.remask(parameters, aggregateKey, maskedCard, alpha2);
      
      // Verify remasking proof
      const isValid = await dlCards.verifyRemask(parameters, aggregateKey, maskedCard, remaskedCard, remaskProof);
      expect(isValid).toBe(true);
    });
  });
  ...
  describe('Shuffle Reproduction', () => {
    it('should reproduce exact shuffle sequence from vector', async () => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const alpha = rng.nextScalar();
        const [maskedCard] = await dlCards.mask(parameters, aggregateKey, card, alpha);
        initialDeck.push(maskedCard);
        maskingFactors.push(alpha);
      }
      
      // Apply shuffles in the same order as the vector
      let currentDeck = initialDeck;
      const shuffleSequence = getShuffleSequence(testVector);
      
      for (const { player } of shuffleSequence) {
        ...
        const newMaskingFactors = Array.from({ length: 4 }, () => rng.nextScalar());
  ...
  describe('Reveal Token Reproduction', () => {
    it('should generate and verify reveal tokens', async () => {
      ...
      // Mask the card
      const [maskedCard] = await dlCards.mask(parameters, aggregateKey, testCard, alpha);
      
      // Generate reveal tokens from all players
      const revealTokens: Array<[any, any, PlayerPublicKey]> = [];
      
      for (const [playerName, [pk, sk]] of playerKeys) {
        const [token, proof] = await dlCards.computeRevealToken(parameters, sk, pk, maskedCard);
        
        // Verify the reveal token
        const isValid = await dlCards.verifyReveal(parameters, pk, token, maskedCard, proof);
        expect(isValid).toBe(true);
      ...
      // Unmask the card
      const unmaskedCard = await dlCards.unmask(parameters, revealTokens, maskedCard);
  ...
  describe('Integration Test - Complete Protocol', () => {
    it('should run complete protocol using vector inputs and match final results', async () => {
      ...
      // 4. Apply vector shuffle sequence
      ...
      for (const finalCard of finalCards) {
        const matchingInitialCard = initialCards.find(({ card }) => 
          card.point.x === finalCard.point.x && card.point.y === finalCard.point.y
        );
      ...
      console.log('✅ Complete protocol executed successfully with vector inputs');
      console.log('✅ All cryptographic proofs verified');
      console.log('✅ Card integrity maintained throughout protocol');
    });
  });
  ...
  describe('Deterministic RNG Verification', () => {
    ...
    it('should generate valid scalars in correct range', () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        expect(scalar).toBeLessThan(BigInt('0x800000000000011000000000000000000000000000000000000000000000001'));
      }
    });
  });
}); 