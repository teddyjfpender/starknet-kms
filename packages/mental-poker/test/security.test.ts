...
describe("Mental Poker Security Tests", () => {
  ...
  describe("Cryptographic Soundness", () => {
    ...
    it("should reject invalid reveal proofs", async () => {
      const testCard = { point: pp.generators.G, index: 0 as any }
      const alpha = randScalar()
      const [maskedCard] = await protocol.mask(
        pp,
        aggregateKey,
        testCard,
        alpha,
      ...
      const isValid = await protocol.verifyReveal(
        pp,
        player.pk,
        revealToken,
        maskedCard,
    ...
    it("should prevent shuffle proof forgery", async () => {
      ...
      const maskingFactors = Array.from({ length: deckSize }, () =>
        randScalar(),
      )
      const permutation = createPermutation([3, 1, 0, 2]) // Valid permutation

      const [, shuffleProof] = await protocol.shuffleAndRemask(
        pp,
        aggregateKey,
        deck,
        maskingFactors,
  ...
  describe("Zero-Knowledge Properties", () => {
    it("should generate different proofs for same input", async () => {
      ...
      const [maskedCard1, proof1] = await protocol.mask(
        pp,
        aggregateKey,
        testCard,
        alpha,
      )
      const [maskedCard2, proof2] = await protocol.mask(
        pp,
        aggregateKey,
        testCard,
        alpha,
      ...
      const valid1 = await protocol.verifyMask(
        pp,
        aggregateKey,
        testCard,
        maskedCard1,
      ...
      const valid2 = await protocol.verifyMask(
        pp,
        aggregateKey,
        testCard,
        maskedCard2,
    ...
    it("should not leak information about private inputs", async () => {
      ...
      const [, proof1] = await protocol.mask(pp, aggregateKey, card1, alpha)
      const [, proof2] = await protocol.mask(pp, aggregateKey, card2, alpha)
  ...
  describe("Protocol Robustness", () => {
    it("should handle malformed parameters gracefully", async () => {
      ...
      await expect(
        protocol.mask(invalidParams, aggregateKey, testCard, alpha),
    ...
    it("should validate permutation correctness", async () => {
      ...
      expect(() => createPermutation(invalidMapping)).toThrow(MentalPokerError)
    })
    ...
    it("should require all player reveal tokens for unmasking", async () => {
      ...
      await expect(
        protocol.unmask(pp, partialRevealTokens, maskedCard),
      ).rejects.toThrow(MentalPokerError)
    })
  })
  ...
  describe("Side-Channel Resistance", () => {
    it("should have consistent timing for proof verification", async () => {
      const testCard = { point: pp.generators.G, index: 0 as any }
      const alpha = randScalar()
      const [maskedCard, validProof] = await protocol.mask(
        pp,
        aggregateKey,
        testCard,
        alpha,
      ...
      for (let i = 0; i < 10; i++) {
        ...
        await protocol.verifyMask(
          pp,
          aggregateKey,
          testCard,
          maskedCard,
          i % 2 === 0 ? validProof : invalidProof,
        )
    ...
    it("should not leak information through error messages", async () => {
      const testCard = { point: pp.generators.G, index: 0 as any }
      const alpha = randScalar()
      const [maskedCard, proof] = await protocol.mask(
        pp,
        aggregateKey,
        testCard,
        alpha,
      ...
      for (const invalidProof of invalidProofs) {
        const result = await protocol.verifyMask(
          pp,
          aggregateKey,
          testCard,
          maskedCard,
    ...
    it("should use fresh randomness for each proof", async () => {
      ...
      for (let i = 0; i < proofs.length; i++) {
        for (let j = i + 1; j < proofs.length; j++) {
          ...
          expect(proofs[i]!.commitmentG.equals(proofs[j]!.commitmentG)).toBe(
            false,
          )
        }
      }
  ...
  describe("Input Validation", () => {
    it("should reject invalid card indices", async () => {
      ...
      await expect(
        protocol.mask(pp, aggregateKey, invalidCard, alpha),
      ).rejects.toThrow(MentalPokerError)
    })
    ...
    it("should validate scalar ranges", async () => {
      ...
      await expect(
        protocol.mask(pp, aggregateKey, testCard, invalidAlpha),
      ).rejects.toThrow()
    })
  ...
  describe("Protocol Completeness", () => {
    it("should successfully complete full card lifecycle", async () => {
      ...
      // 1. Mask the card
      const [maskedCard, maskProof] = await protocol.mask(
        pp,
        aggregateKey,
        testCard,
        alpha,
      ...
      const [remaskedCard, remaskProof] = await protocol.remask(
        pp,
        aggregateKey,
        maskedCard,
        beta,
