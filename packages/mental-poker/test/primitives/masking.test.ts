...
import { addPoints, arePointsEqual, scalarMultiply } from "@starkms/crypto"
  ...
  describe("Card Masking with Test Vectors", () => {
    it("should mask cards with known randomness to match test vectors", () => {
      for (const vector of testVectors.test_vectors) {
        ...
        const playerSecretKeys = revealTest.player_secret_keys.map(
          (sk: string) => ElGamal.hexToScalar(sk),
        )
        let aggregatePublicKey = scalarMultiply(
          playerSecretKeys[0]!,
          elgamalParams.generator,
        )
        for (let i = 1; i < playerSecretKeys.length; i++) {
          const playerPubKey = scalarMultiply(
            playerSecretKeys[i]!,
            elgamalParams.generator,
          )
        ...
        const { maskedCard, proof } = Masking.mask(
          elgamalParams,
          publicKey,
          card,
          maskingFactor,
    ...
    it("should verify masking proofs from test vectors", () => {
      for (const vector of testVectors.test_vectors) {
        ...
        const isValid = Masking.verifyMask(
          elgamalParams,
          publicKey,
          card,
          maskedCard,
  ...
  describe("Card Remasking with Test Vectors", () => {
    it("should remask cards with known randomness to match test vectors", () => {
      for (const vector of testVectors.test_vectors) {
        ...
        const additionalRandomness = ElGamal.hexToScalar(
          remaskingTest.additional_masking_factor,
        )

        // Remask with known randomness
        const { remaskedCard, proof } = Masking.remask(
          elgamalParams,
          publicKey,
          originalMasked,
        ...
        expect(ElGamal.pointToHex(remaskedCard.c1)).toBe(
          remaskingTest.remasked_card.c1,
        )
        expect(ElGamal.pointToHex(remaskedCard.c2)).toBe(
          remaskingTest.remasked_card.c2,
        )
    ...
    it("should verify remasking proofs from test vectors", () => {
      for (const vector of testVectors.test_vectors) {
        ...
        const proof: Masking.MaskingProof = {
          commitment: ElGamal.hexToPoint(
            remaskingTest.remasking_proof.commitment,
          ),
          challenge: ElGamal.hexToScalar(
            remaskingTest.remasking_proof.challenge,
          ),
          response: ElGamal.hexToScalar(remaskingTest.remasking_proof.response),
        }

        // Verify the remasking proof
        const isValid = Masking.verifyRemask(
          elgamalParams,
          publicKey,
          originalMasked,
          remaskedCard,
  ...
  describe("Card Revealing with Test Vectors", () => {
    ...
    it("should unmask cards using reveal tokens from test vectors", () => {
      for (const vector of testVectors.test_vectors) {
        ...
        const unmaskedCard = Masking.unmask(
          elgamalParams,
          revealTokens,
          maskedCard,
        )
