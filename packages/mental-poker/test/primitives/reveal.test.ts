...
describe("Reveal Implementation with Test Vectors", () => {
  ...
  describe("Core Reveal Operations", () => {
    test("should compute reveal tokens to match test vectors", () => {
      for (const testVector of testVectors.test_vectors) {
        ...
        const actualRevealToken = Reveal.computeRevealToken(
          pp,
          secretKey,
          maskedCard,
        )
        ...
        expect(
          arePointsEqual(actualRevealToken.point, expectedRevealToken),
        ).toBe(true)
      }
    })
    ...
    test("should generate valid reveal proofs", () => {
      for (const testVector of testVectors.test_vectors) {
        ...
        // Generate proof
        const proof = Reveal.proveReveal(
          pp,
          secretKey,
          publicKey,
          maskedCard,
        ...
        // Verify the proof
        const isValid = Reveal.verifyReveal(
          pp,
          publicKey,
          maskedCard,
          revealToken,
  ...
  describe("Combined Operations", () => {
    test("should perform reveal token computation with proof generation", () => {
      for (const testVector of testVectors.test_vectors) {
        ...
        const expectedRevealToken = hexToPoint(testVector.reveal_token)
        expect(arePointsEqual(revealToken.point, expectedRevealToken)).toBe(
          true,
        )
      }
    })
  ...
  describe("Multi-Party Reveal", () => {
    test("should support multi-party reveal operations", () => {
      ...
      const revealTokens: Reveal.RevealToken[] = [{ point: baseToken }]

      // Perform multi-party reveal with single token
      const revealedPlaintext = Reveal.multiPartyReveal(
        revealTokens,
        maskedCard,
      )
  ...
  describe("Compatibility with Other Modules", () => {
    test("should be compatible with existing masking operations", () => {
      ...
      const revealToken = Reveal.computeRevealToken(
        elgamalParams,
        revealSecretKey,
        revealMaskedCard,
      )
    ...
    test("should demonstrate full masking/revealing cycle", () => {
      ...
      // Verify the proof
      const isProofValid = Reveal.verifyReveal(
        pp,
        publicKey,
        revealMaskedCard,
        revealToken,
