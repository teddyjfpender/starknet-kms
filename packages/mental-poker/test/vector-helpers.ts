...
export function extractShuffleSequences(vector: any): number[][] {
  ...
  // Extract permutations from shuffle_sequence
  if (vector.shuffle_sequence && Array.isArray(vector.shuffle_sequence)) {
    for (const shuffle of vector.shuffle_sequence) {
      if (shuffle.permutation && Array.isArray(shuffle.permutation)) {
        sequences.push(shuffle.permutation)
      }
    }
  }

  // If no sequences found, return a simple test permutation
  if (sequences.length === 0) {
    sequences.push([3, 1, 0, 2]) // Simple 4-element permutation for testing
  }
...
export function extractMaskingFactors(vector: any): bigint[][] {
  ...
  // Extract masking factors from shuffle_sequence
  if (vector.shuffle_sequence && Array.isArray(vector.shuffle_sequence)) {
    for (const shuffle of vector.shuffle_sequence) {
      if (shuffle.masking_factors && Array.isArray(shuffle.masking_factors)) {
        const factors = shuffle.masking_factors.map(
  ...
  // If no masking factors found, generate some for testing
  if (maskingFactors.length === 0) {
    // Create a simple set of masking factors for 4 cards
    const testFactors = [
      BigInt(
    ...
    maskingFactors.push(testFactors)
  }
