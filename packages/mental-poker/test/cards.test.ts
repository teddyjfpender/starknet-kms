...
class TestPlayer {
  ...
  async revealCard(
    ...
    const unmaskedCard = await protocol.unmask(
      pp,
      allRevealTokens,
      maskedCard,
      cardEncoding,
...
describe("Mental Poker Protocol - Card Masking", () => {
  ...
  it("should remask and verify card remasking", async () => {
    ...
    const [remaskedCard, proof] = await protocol.remask(
      pp,
      sharedKey,
      maskedCard,
      remaskingFactor,
    ...
    const isValid = await protocol.verifyRemask(
      pp,
      sharedKey,
      maskedCard,
      remaskedCard,
  ...
  it("should compute and verify reveal tokens", async () => {
    ...
    const [revealToken, proof] = await protocol.computeRevealToken(
      pp,
      player.sk,
      player.pk,
      maskedCard,
...
describe("Mental Poker Protocol - Card Shuffling", () => {
  ...
  it("should shuffle and verify deck", async () => {
    ...
    const maskingFactors = Array.from({ length: maskedDeck.length }, () =>
      randScalar(),
    )
...
describe("Mental Poker Protocol - Card Revealing", () => {
  ...
  it("should fail to reveal with insufficient tokens", async () => {
    ...
    const correctCard = await protocol.unmask(
      pp,
      allRevealTokens,
      maskedCard,
      cardEncoding,
    ...
    const partialCard = await protocol.unmask(
      pp,
      partialRevealTokens,
      maskedCard,
      cardEncoding,
...
describe("Mental Poker Protocol - Full Game Simulation", () => {
  ...
  it("should simulate a complete poker round", async () => {
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      const maskingFactors = Array.from({ length: deck.length }, () =>
        randScalar(),
      )
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      // Reveal the card
      const revealedCard = await player.revealCard(
        protocol,
        pp,
        playerCard,
        revealTokens,
      ...
      expect(
        initialCards.some(
          (card) =>
            card.value === revealedCard.value &&
            card.suite === revealedCard.suite,
        ),
      ).toBe(true)
    }
...
describe("Mental Poker Protocol - Security Properties", () => {
  ...
  it("should verify zero-knowledge proofs correctly", async () => {
    ...
    const keyValid = await protocol.verifyKeyOwnership(
      pp,
      player.pk,
      playerInfo,
      keyProof,
    ...
    const wrongKeyValid = await protocol.verifyKeyOwnership(
      pp,
      player.pk,
      wrongInfo,
      keyProof,
...
describe("Mental Poker Protocol - Error Handling", () => {
  ...
  it("should handle cryptographic errors gracefully", async () => {
    ...
    await expect(protocol.playerKeygen(invalidPp)).rejects.toThrow()
  })
})
