...
describe("Mental Poker Performance Benchmarks", () => {
  ...
  describe("Setup Performance", () => {
    ...
    it("should scale reasonably with player count", async () => {
      ...
      console.log(
        "Setup times by player count:",
        times
          .map((t, i) => `${playerCounts[i]} players: ${t.toFixed(2)}ms`)
          .join(", "),
      )
  ...
  describe("Card Operations Performance", () => {
    ...
    it("should verify masking proofs quickly", async () => {
      ...
      console.log(
        `Average masking proof verification time: ${avgTime.toFixed(2)}ms`,
      )
    ...
    it("should handle reveal token generation efficiently", async () => {
      ...
      const avgTime = (end - start) / iterations
      console.log(
        `Average reveal token generation time: ${avgTime.toFixed(2)}ms`,
      )
  ...
  describe("Shuffle Performance", () => {
    it("should shuffle small decks quickly", async () => {
      ...
      const start = performance.now()
      await protocol.shuffleAndRemask(
        pp,
        aggregateKey,
        deck,
        maskingFactors,
    ...
    it("should verify shuffle proofs in reasonable time", async () => {
      ...
      console.log(
        `4-card shuffle verification time: ${verifyTime.toFixed(2)}ms`,
      )
  ...
  describe("Full Game Simulation Performance", () => {
    it("should complete a poker hand in reasonable time", async () => {
      ...
      // 2. Shuffle the deck
      const maskingFactors = Array.from({ length: numCards }, () =>
        randScalar(),
      )
      ...
      for (let i = shuffleMapping.length - 1; i > 0; i--) {
        ...
        ;[shuffleMapping[i], shuffleMapping[j]] = [
          shuffleMapping[j]!,
          shuffleMapping[i]!,
        ]
      }
      ...
      console.log(
        `Complete poker hand simulation time: ${totalTime.toFixed(2)}ms`,
      )
  ...
  describe("Concurrent Performance", () => {
    it("should handle concurrent operations efficiently", async () => {
      ...
      console.log(
        `Average time per concurrent operation: ${avgTime.toFixed(2)}ms`,
      )
  ...
  describe("Scalability Tests", () => {
    it("should scale with deck size", async () => {
      ...
      console.log(
        "Deck creation times:",
        times
          .map((t, i) => `${deckSizes[i]} cards: ${t.toFixed(2)}ms`)
          .join(", "),
      )

      // Should scale linearly with deck size
      const scalingRatio =
        times[3]! / times[0]! / (deckSizes[3]! / deckSizes[0]!)
